const path = require('path')
    ...
    , session = require('express-session')                               // ATTENTION: pas fait pour la production, juste pour le développement et le débugage. En production, voire: 'connect-pg-simple'
    , sessionStore = session.MemoryStore({ reapInterval: 60000 * 10 })
...
app.engine('ejs', require('ejs-locals'));                    // requires the engine for the file extension. Ici, ejs --> ejs-locals
app.set('views', path.join(__dirname, 'views'));             // on ajoute un chemin
app.set('view engine', 'ejs');                               // pas sûr de ce que ça fait mais ça plante pas...
app.use(express.static(path.join(__dirname, '/public')));    // app.use: charge une fonction comme middleware, app.static: prend en entrée un chemin et en ressort un middleware qui sert pour tous les fichiers dans le chemin indiqué
...
app.use(bodyParser.json());         // permet de parser les cookies 
//});
//app.configure('development', function(){
//app.use(app.use(errorHandler()));         -------> probleme a regler
//});
//app.configure('production', function(){
app.use(errorHandler());            // ça on sait pas
//});

/** Routes */
app.get('/session-index', function (req, res, next) {        // la page /session-index
...
function requireLogin (req, res, next) {
  if (req.session.username) {
    ...
    res.redirect("/login");                            // s'il veut s'identifier, on le redirige vers /login
  }
}

/** Home page (requires authentication) */
app.get('/', [requireLogin], function (req, res, next) {              // page principale: identification nécessaire
  res.render('index', { "username": req.session.username });
});

/** Login form */
app.get("/login", function (req, res) {                               // la page de login
...
app.post("/login", function (req, res) {                              // post a request to the server
  ...
  if (!req.body.username) {
    ...
    res.redirect("/");
  } else if (!req.body.username.match(/^[a-zA-Z0-9\-_]{3,}$/)) {      // le user utilise des caractères interdits
    options.error = "User name must have at least 3 alphanumeric characters";
    res.render("login", options);
  } else {                                                            // si le username est libre --> valide
